...
// on sucess, returns 0.
// on failure returns -1.
int NXT_send_buffer(NXT *nxt,msg_t b,unsigned short int len){
...
int NXT_recv_buffer(NXT *nxt,msg_t b,unsigned short int len){
    char l2[2];
...
void NXT_play_tone(NXT *nxt,unsigned short int freq,unsigned short int time,
    ...
    msg_t msg;
    if(ans){
        msg = alloc_msg_t(0x0,0x3);
    } else {
        msg = alloc_msg_t(0x80,0x3);
    }
    msg_addU16(msg,freq);
    msg_addU16(msg,time);
    NXT_send_buffer(nxt,msg,msg_len(msg));
    if(ans){
	char buf[3];
...
unsigned short int NXT_get_battery_level(NXT* nxt){
    msg_t msg = alloc_msg_t(0,0x0b);
    char buf[5];
    NXT_send_buffer(nxt,msg,msg_len(msg));
...
void NXT_set_motorstate(NXT *nxt,motorstate_t st,int ans,
    ...
    msg_t msg;
    if(ans){
        msg = alloc_msg_t(0x0,0x4);
    } else {
        msg = alloc_msg_t(0x80,0x4);
    }
...
void NXT_set_input_mode(NXT *nxt,unsigned char port,sensor_type type,
    ...
    msg_t msg;
    if(ans){
	msg = alloc_msg_t(0x0,0x5);
    } else {
	msg = alloc_msg_t(0x80,0x5);
    }
    msg_addU8(msg,port);
    msg_addU8(msg,type);
    msg_addU8(msg,mode);
...
void NXT_reset_motor_position(NXT *nxt,unsigned char port,
    ...
    msg_t msg;
    if(ans){
        msg = alloc_msg_t(0x0,0xa);
    }else{
        msg = alloc_msg_t(0x80,0xa);
    }
    msg_addU8(msg,port);
    if(relative){
        msg_addU8(msg,1);
...
void NXT_stay_alive(NXT* nxt){
    msg_t msg = alloc_msg_t(0x80,0x0D);
    NXT_send_buffer(nxt,msg,2);
    free(msg);
}
